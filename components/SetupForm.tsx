...
export default function SetupForm({ onFetch, loading }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTokenSaved(true);
    onFetch(token.trim(), "hbs.instructure.com", startDate, endDate);
  }

  function handleClearToken() { localStorage.removeItem(TOKEN_KEY); setToken(""); setTokenSaved(false); }

  const tokenSettingsUrl = "https://hbs.instructure.com/profile/settings";
        ...
        <div>
          ...
          {showInstructions && (
            <div className="mb-3 bg-blue-50 border border-blue-100 rounded-lg p-4 text-xs text-gray-600 space-y-1.5">
              ...
              <ol className="list-decimal list-inside space-y-1">
                ...
                <li>Give it a name (e.g. &quot;Canvas Sync&quot;) â leave expiry blank</li>
                <li>Click <strong>Generate Token</strong> and copy it immediately</li>
              </ol>
              <p className="text-gray-500 mt-1">Your token is stored only in your browser and sent directly to Canvas â never to any third-party server.</p>
            </div>
          )}
          ...
          <div className="mt-1 flex items-center justify-between">
            {tokenSaved
              ...
              : <p className="text-xs text-gray-400">Saved locally â never sent to any server.</p>}
            {tokenSaved && (
